...
\irust{vec![]}, které bere počáteční hodnoty.
...
\irust{u8}. Pokud chceme používat ASCII, můžeme používat \uv{byte string} (\irust{b""}).
Jeho charaktery můžeme psát jako \irust{b'a'} apod.
...
Pokud chceme přidat \uv{metody}, uděláme tak v~tzv. \irust{impl} bloku. Můžeme zde
vytvářet funkce, které jako první parametr berou \irust{self}, \irust{&self} nebo
...
\end{rustcode}
\endobrl{Ukázka impl bloku}{struct impl showcase}
